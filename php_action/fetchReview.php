...
if (empty($response)) {
  ...
  $sql = "INSERT INTO rboard (location, user_id, name, subject, content, imglist, rate, ip, rdate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";
  ...
  if (!$stmt) {
    ...
    $imglist = '';
    $stmt->bind_param('sissssss', $location, $userId, $name, $title,  $contentWithPaths, $imglist, $rate, $ip);
    if ($stmt->execute()) {
